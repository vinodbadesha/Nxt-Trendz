...
class App extends Component {
        ...
        cartList: prevState.cartList.map(eachItem => {
          if (productObject.id === eachItem.id) {
            const updatedProductQuantity =
              productObject.quantity + eachItem.quantity
            return {...eachItem, quantity: updatedProductQuantity}
          }
          return eachItem
        }),
      }))
    } else {
      const updatedCartItem = [...cartList, product]
  ...
  removeCartItem = id => {
    const {cartList} = this.state
    const filteredCartListById = cartList.filter(
      eachCartItem => eachCartItem.id !== id,
    )

    this.setState({cartList: filteredCartListById})
  }
